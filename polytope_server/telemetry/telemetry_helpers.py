...
def histogram_metric_names(base: str) -> Tuple[str, str, str, str]:
    """
    Returns (base, base_bucket, base_sum, base_count) for Prometheus exposition.
    """
...
def parse_window(window: Optional[str], default_seconds: float = 300.0) -> float:
    ...
    try:
        ...
        val = float(num)
        mult = {"s": 1, "m": 60, "h": 3600, "d": 86400}[unit] if unit in {"s", "m", "h", "d"} else 1
...
def render_samples(name: str, rows: Iterable[Mapping[str, Any]]) -> List[str]:
    out: List[str] = []
    for row in rows:
        labels = row.get("labels", {})
        value = row.get("value", 0)
        out.append(f"{name}{labels_to_exposition(labels)} {value}")
    return out


def build_product_labels(doc: Mapping[str, Any]) -> Dict[str, Any]:
    """
    Construct metric labels from a request-like document:

    - realm from user.realm,
    - product shape from coerced_request using TELEMETRY_PRODUCT_LABELS,
    - collection as-is.

    This intentionally avoids high-cardinality fields
    """
    labels: Dict[str, Any] = {
...
def seconds_to_duration_label(seconds: int) -> str:
    """
    Render a duration seconds as a human-friendly label aligned with PromQL notation:
    - 86400-multiples as Nd
    - 3600-multiples as Nh
    - 60-multiples as Nm
    - otherwise Ns
    """
    if seconds % 86400 == 0:
